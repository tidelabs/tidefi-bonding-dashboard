  ...
  <q-layout view="lHh Lpr lFf">
    <q-header elevated class="glass">
      <q-toolbar>
        ...
        <div
          class="column q-toolbar__title text-right ellipsis"
          style="font-size: 12px"
        >
          <div class="ellipsis">
            {{ clientStore.nodeName }} {{ clientStore.nodeVersion }}
          </div>
          <div class="ellipsis">
            {{ clientStore.consts.version.specName }}/{{
              clientStore.consts.version.specVersion
            }}
          </div>
        </div>
...
export default defineComponent({
  ...
  setup () {
    ...
    onBeforeMount(() => {
      ...
      if (val) {
        chainStore.chainName = val
      }
      else {
        chainStore.chainName = chainStore.chains[ 0 ].name
      }
    ...
    watch(chainName, async () => {
      const chain = chainStore.chains.find((chain, index) => {
        if (chain.name === chainName.value) {
          chainStore.chainName = chainName.value
          chainStore.chainIndex = index
          return chain
        }
        return undefined
      })
      ...
      $q.localStorage.set(
        'chainName',
        chainStore.chains[ chainStore.chainIndex ].name
      )
      console.log(
        `chain specs changed to: ${
          chainStore.chains[ chainStore.chainIndex ].name
        } (${ chainStore.chains[ chainStore.chainIndex ].rpc })`
      )
    ...
    function setDefaultChain () {
      chainStore.chainName = chainStore.chains[ 0 ].name
      chainStore.chainIndex = 0
      chainStore.client = null
    }

    watch(selectedNetwork, (val) => {
      // console.log('selectedNetwork changed:', selectedNetwork.value)
      // verify different network
      if (chainStore.chainName === val.name) {
        return
      }

      chainStore.chainName = val.name
    })
    ...
    //   const validators = await chainStore.client.api.query.session.validators()
    ...
    //   // const disabledValidators = await chainStore.client.api.query.session.disabledValidators()
    //   // if (disabledValidators && disabledValidators.length > 0) {
    //   //   disabledValidators.forEach(async (val) => {
    //   //     // console.log('validator:', val)
    //   //     const validator = new Entity(chainStore.client, val.toString(), val, true)
    //   //     await validator.connect()
