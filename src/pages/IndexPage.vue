<template>
  <q-page>
    <div class="row justify-evenly items-stretch col-xs-12 col-sm-6 col-md-4 col-lg-3">
      <CurrentBlock />
      <CurrentEpoch />
      <ActiveEra />
      <TotalIssuance />
    ...
    <template v-slot:top>
      <div class="row justify-center items-center q-gutter-sm">
        <div>Filters:</div>
        <q-btn label="High Commission" :icon="scalesDuotone" no-caps rounded outline @click="filters.lowCommission = !filters.lowCommission" :class="{ 'filter-button-active': filters.lowCommission }" />
        <q-btn label="Inactive Validators" :icon="alarmClockOutline" no-caps rounded outline @click="filters.inactive = !filters.inactive" :class="{ 'filter-button-active': filters.inactive }" />
        <q-btn label="Oversubscribed" :icon="notificationImportantTwoTone" no-caps rounded outline @click="filters.oversubscribed = !filters.oversubscribed" :class="{ 'filter-button-active': filters.oversubscribed }" />
        <q-btn label="Blocked Nominations" :icon="personAddDisabledTwoTone" no-caps rounded outline @click="filters.blockedNominations = !filters.blockedNominations" :class="{ 'filter-button-active': filters.blockedNominations }" />
        <q-btn label="Missing Identity" :icon="identityTwoTone" no-caps rounded outline @click="filters.missingIdentity = !filters.missingIdentity" :class="{ 'filter-button-active': filters.missingIdentity }" />
      </div>
    </template>
    <template v-slot:body="props">
      <q-tr :props="props">

        <q-td key="identicon" :props="props">
          <div><span v-html="props.row.identicon" /></div>
        </q-td>
    ...
    <template v-slot:item="props">
      <q-markup-table class="q-ma-md" style="min-width: 300px;">
        <tbody>
          <tr>
            <td></td>
            <td><span v-html="props.row.identicon" /></td>
          </tr>
...
import TotalValidators from 'components/TotalValidators.vue'
import TotalNominators from 'components/TotalNominators.vue'
import CurrentBlock from 'components/CurrentBlock.vue'
import TotalIssuance from 'components/TotalIssuance.vue'
